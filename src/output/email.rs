...
pub struct Config {
    smtp_server: String,
    #[serde(default = "default_smtp_port")] smtp_port: u16,
    #[serde(default = "default_smtp_username")] smtp_username: String,
    #[serde(default = "default_smtp_password")] smtp_password: String,
...
impl OutputPlugin for Plugin {
    fn remind(&self,
        ...
        let mail = EmailBuilder::new()
            .from((self.config.from_address.as_ref(), self.config.from_name.as_ref()))
            .to(self.config.to_address.as_ref())
            .subject(subject.as_ref())
            .html(body.as_ref())
            .build()?;

        let mut mailer = SmtpTransportBuilder::new((self.config.smtp_server.as_ref(), self.config.smtp_port))?
